...
import { Task, TaskStatus, DragEndResult } from '../types/task';
...
const TaskBoard = ({ onEditTask, refreshTasks }: TaskBoardProps) => {
  ...
  if (tasks.length === 0) {
    return (
      <div className="flex justify-center items-center h-64">
        <div className="text-gray-600">No tasks found. Create a new task to get started!</div>
      </div>
    );
  }
  ...
  return (
    <DragDropContext onDragStart={handleDragStart} onDragEnd={handleDragEnd}>
      <div className={`grid grid-cols-1 md:grid-cols-3 gap-4 p-4 transition-opacity duration-200 ${isDragging ? 'opacity-75' : 'opacity-100'}`}>
        {Object.entries(columns).map(([status, columnTasks]) => (
          <div key={status} className="bg-gray-100 rounded-lg p-4">
            <h2 className="text-xl font-bold mb-4 capitalize">{status.toLowerCase().replace('_', ' ')}</h2>
